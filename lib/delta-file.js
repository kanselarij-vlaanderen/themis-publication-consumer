...
class DeltaFile {
  constructor(data) {
    ...
    this.subject = data.attributes.subject;
  }
  ...
  async ingest(onFinishCallback) {
    ...
    try {
      ...
      for (let { inserts, deletes } of changeSets) {
        ...
        downloadFailedCount += await syncDocuments(inserts, temporaryGraph);
      }
      await scheduleReleaseTask(temporaryGraph);
      console.log(`Successfully finished ingesting delta file ${this.id} stored at ${this.tmpFilepath}`);
      await onFinishCallback(this, true, downloadFailedCount); // failed documents should not interrupt the flow or mark the ingestion as failed
      await fs.unlink(this.tmpFilepath);
    } catch (e) {
      console.log(`Something went wrong while ingesting delta file ${this.id} stored at ${this.tmpFilepath}`);
      console.log(e);
      await onFinishCallback(this, false);
    }
  }
...
async function scheduleReleaseTask(sourceGraph) {
  const id = uuid();
    ...
    INSERT DATA {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        <${uri}> a ext:ReleaseTask ;
           ...
           ${this.subject ? `dct:subject ${sparqlEscapeUri(this.subject)} ;` : ''}
           dct:source <${sourceGraph}> .
      }
    }
  `);
  console.log(`Created new release task <${uri}> for graph <${sourceGraph}>.`);
}
