...
class DeltaFile {
  ...
  async ingest(onFinishCallback) {
    ...
    try {
      ...
      const temporaryGraph = getTemporaryGraph();
      ...
      for (let { inserts, deletes } of changeSets) {
        ...
        await deleteTriples(deletes); // TODO: can this be removed?
...
function getTemporaryGraph() {
  return MU_TMP_BASE_GRAPH_URI + '' + Date.now()
}
...
async function scheduleReleaseTask(graph, id) {
  await update(`
    ...
    INSERT DATA {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        <${MU_APPLICATION_GRAPH}/${id}> a ext:ReleaseTask ; 
           mu:uuid "${id}" ;
