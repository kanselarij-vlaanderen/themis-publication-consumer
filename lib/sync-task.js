...
import mu, { sparqlEscapeDateTime, uuid, sparqlEscapeUri } from 'mu';
import { createEmailOnFailure } from './email';

import { MU_APPLICATION_GRAPH, SYNC_TASK_STATUSES, START_FROM_DELTA_TIMESTAMP, SYNC_BASE_URL } from '../config';
...
class SyncTask {
  constructor({ uri, since, until, created, status }) {
    ...
    */
    ...
    /**
     * If we hit any issues that do not result into thrown errors
     * Empty if task was executed successfully
    */
    this.errorMessage = '';

    /**
     * Count of the documents that failed to download from Kaleidos
     * Could be a size issue or connection, does not block task progress
     */
  ...
  */
  async execute() {
    try {
      await this.persistStatus(SYNC_TASK_STATUSES.ONGOING);
      console.log('***list of subjects: ',this.files.map((file) => file.subject));
      await this.persistSubjects(this.files.map((file) => file.subject));
      ...
      if (this.totalFiles) {
        ...
        await this.persistStatus(SYNC_TASK_STATUSES.SUCCESS);
      }
    } catch (e) {
  ...
  */
  async closeWithFailure() {
    await this.persistLatestDelta(this.latestDeltaMs);
    await this.persistStatus(SYNC_TASK_STATUSES.FAILED);
  }
  ...
  */
  async consumeNext() {
    ...
    try {
      await file.consume(async (file, isSuccess, downloadFailedCount = 0) => {
        if (isSuccess) {
          this.handledFiles++;
        ...
        if (this.progressStatus == 'progressing' && this.handledFiles < this.totalFiles) {
          ...
          if (this.progressStatus == 'failed') {
            await this.persistStatus(SYNC_TASK_STATUSES.FAILED);
            this.errorMessage = `Failed to finish sync task. Skipping the remaining files. Most recent delta file successfully consumed is created at ${this.latestDelta.toISOString()}.`;
            ...
            if (this.downloadFailedCount) {
              // task can succeed with document download failures
              this.errorMessage = `Finished sync task successfully but ${this.downloadFailedCount} documents could not be downloaded.`
              console.log(this.errorMessage);
            }
          }
          ...
          if (this.progressStatus == 'failed' || this.downloadFailedCount) {
            await createEmailOnFailure(
              "A sync task has partially failed in themis-publication-consumer",
              `environment: ${SYNC_BASE_URL}\t\Detail of error: ${this.errorMessage}}`
            );
          }
        }
      });
    } catch (e) {
      console.log(`Something went wrong while consuming next`);
      throw e;
    }
  };
  ...
  */
  ...
  /**
   * Persists the subjects from delta files in the triple store
   *
   * @private
  */
  async persistSubjects(subjects) {
    const statements = subjects?.map(subject => {
      return returnsubject ? `${sparqlEscapeUri(this.uri)} dct:subject ${sparqlEscapeUri(subject)} .` : '';
    }).join('\n');
    ...
    const query = `
      ...
      INSERT {
        ...
        GRAPH ${sparqlEscapeUri(MU_APPLICATION_GRAPH)} {
          ${sparqlEscapeUri(this.uri)} a ext:SyncTask .
        }
      }
    `
...
async function scheduleSyncTask() {
  const result = await query(`
    ...
    SELECT ?s WHERE {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        ?s a ext:SyncTask ;
          adms:status <${SYNC_TASK_STATUSES.NOT_STARTED}> .
      }
    } LIMIT 1
  `);
  ...
  if (result.results.bindings.length) {
    ...
    const uuid = mu.uuid();
    const uri = `http://kanselarij.vo.data.gift/services/themis-publication-consumer/sync-tasks/${uuid}`;
    await update(`
      PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
      PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
      PREFIX adms: <http://www.w3.org/ns/adms#>
...
async function getNextSyncTask() {
  ...
  if (result.results.bindings.length) {
    ...
    return new SyncTask({
      uri: b['s'].value,
      status: SYNC_TASK_STATUSES.NOT_STARTED,
...
async function getRunningSyncTask() {
  const result = await query(`
    ...
    SELECT ?s WHERE {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        ?s a ext:SyncTask ;
           adms:status <${SYNC_TASK_STATUSES.ONGOING}> .
      }
    } ORDER BY ?created LIMIT 1
  `);
...
async function setTaskFailedStatus(uri) {
  ...
  await update(`
    ...
    INSERT {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        <${uri}> adms:status <${SYNC_TASK_STATUSES.FAILED}> .
      }
    } WHERE {
      GRAPH <${MU_APPLICATION_GRAPH}> {
        <${uri}> a ext:SyncTask .
