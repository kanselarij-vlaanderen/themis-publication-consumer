...
const TASK_NOT_STARTED_STATUS = 'http://kanselarij.data.gift/services/valvas-publication-consumer/sync-task-statuses/not-started';
const TASK_ONGOING_STATUS = 'http://kanselarij.data.gift/services/valvas-publication-consumer/sync-task-statuses/ongoing';
const TASK_SUCCESS_STATUS = 'http://kanselarij.data.gift/services/valvas-publication-consumer/sync-task-statuses/success';
const TASK_FAILED_STATUS = 'http://kanselarij.data.gift/services/valvas-publication-consumer/sync-task-statuses/failed';
...
async function scheduleSyncTask() {
  ...
  if (result.results.bindings.length) {
    ...
    const uuid = mu.uuid();
    const uri = `http://kanselarij.data.gift/services/valvas-publication-consumer/sync-tasks/${uuid}`;
    await update(`
      PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
      PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
      PREFIX dct: <http://purl.org/dc/terms/>
...
async function getRunningSyncTask() {
  const result = await query(`
    ...
    SELECT ?s WHERE {
      ?s a ext:SyncTask ;
         ext:taskStatus <${TASK_ONGOING_STATUS}> .
    } ORDER BY ?created LIMIT 1
  `);
 ...
 * @public
*/
async function setRunningSyncTaskToFailed(uri) {
  await update(`
    PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
    PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
    PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>
  ...
  await update(`
    ...
    INSERT {
      GRAPH ?g {
        <${uri}> ext:taskStatus <${TASK_FAILED_STATUS}> .
      }
    } WHERE {
      GRAPH ?g {
        <${uri}> a ext:SyncTask .
