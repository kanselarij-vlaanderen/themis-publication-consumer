  ...
  for (let document of documents) {
    ...
    if (result.results.bindings.length) {
      ...
      try {
        ...
        console.log(`Failed to download document <${locationUri}> from Kaleidos to Themis. This failure will not block the sync flow.`);
        console.log(e);
        downloadFailedCount++;
      }
    }
  }
  console.log(`Copied ${downloadCount} files from Kaleidos to Themis`);
  if (downloadFailedCount) {
    console.log(`Failed to Copy ${downloadFailedCount} files from Kaleidos to Themis`);
  }
